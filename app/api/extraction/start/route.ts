...
export async function POST(req: NextRequest) {
  try {
    ...
    // 4. Vérifier la limite de factures mensuelles du plan
    ...
    if (countError) {
      console.error('❌ Erreur comptage factures mensuelles:', countError);
      ...
      if (!canExtract) {
        ...
        console.warn(`⚠️ Limite mensuelle atteinte: ${currentMonthlyCount}/${limit} factures`);
        return NextResponse.json(
          { 
            error: 'plan_limit_reached',
            feature: 'invoices',
            message: `Vous avez atteint votre limite mensuelle de ${limit} factures. Améliorez votre plan pour extraire plus de factures.`,
